...
clust.assess <- c("cluster.number","n","average.within","average.between",
...
for(i in c(2:k)){
  ...
  for(d in 1:k) {
    cluster.sizes[d, i] <- unlist(cluster.stats(gower.dist, cutree(divisive.clust, i))[clust.size])[d]
...
cstats.table(gower.dist, divisive.clust.c, 5)
cstats.table(gower.dist, aggl.clust.c, 5) #complete linkages looks like the most balanced approach
